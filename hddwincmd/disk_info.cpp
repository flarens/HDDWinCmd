...
// получить все точки монтирования по volume GUID
std::vector<std::wstring> GetMountPoints(const std::wstring& volumeGUID) {
	std::vector<std::wstring> mountPoints;
	DWORD bufferSize = MAX_PATH;
	std::vector<WCHAR> buffer(bufferSize);

	while (!GetVolumePathNamesForVolumeNameW(volumeGUID.c_str(), buffer.data(), bufferSize, &bufferSize)) {
...
// получить метку тома по volume GUID
std::wstring GetVolumeLabel(const std::wstring& volumeGUID) {
...
// получить файловую систему по volume GUID
std::wstring GetFileSystem(const std::wstring& volumeGUID) {
	wchar_t volumeName[MAX_PATH + 1];
	wchar_t fileSystemName[MAX_PATH + 1];
	DWORD volumeSerialNumber, maximumComponentLength, fileSystemFlags;
...
bool GetPhysicalDrive(const std::wstring& devicePath, std::vector<DiskInfo>& logicalDisks, const bool maxinfo, s_resp& resp, const bool single_source) {
	...
	std::wstring devicePathWinPermanent = L"";
	...
	if (deviceNumber == -1) {
		...
		WcoutExt_Mini(L"DevicePath (windows, temporary): \\\\.\\PhysicalDrive" + std::to_wstring(deviceNumber) + L"\r\n", resp, true);
		WcoutExt_Mini(L"DevicePath (linux, temporary): /dev/sd" + NumberTolinuxDriveLetters(deviceNumber) + L"\r\n", resp, true);
		if (single_source) devicePathWinPermanent = GetPermanentDevicePathByDeviceNumber(deviceNumber);
	}
	if (!single_source) devicePathWinPermanent = devicePath;
	if (devicePathWinPermanent != L"") {
		WcoutExt_Mini(L"DevicePath (windows, permanent):\r\n" + devicePathWinPermanent + L"\r\n", resp, true);
		deviceID = ConvertDevicePathToDeviceInstanceId(devicePathWinPermanent);
	}
	if (deviceID != L"") WcoutExt_Mini(L"DeviceID (windows, permanent):\r\n" + deviceID + L"\r\n", resp, true);
		...
		for (DWORD i = 0; i < driveLayout->PartitionCount; ++i) {
			...
			for (const auto& disk : logicalDisks) {
				if (disk.PhysicalDriveNumber == deviceNumber && disk.StartingOffset == partition.StartingOffset.QuadPart) {
					...
					// точки монтирования
					std::vector<std::wstring> mountPoints = GetMountPoints(disk.GUID);
					int mountPointNum = 1;
					for (const auto& mountPoint : mountPoints) {
						WcoutExt_Mini(L"    Mount point (" + std::to_wstring(mountPointNum) + L"): " + mountPoint + L"\r\n", resp, true);
						mountPointNum++;
					}

					// файловая система
					WcoutExt_Mini(L"    File System: " + GetFileSystem(disk.GUID) + L"\r\n", resp, true);
					// метка тома
					WcoutExt_Mini(L"    Volume Label: " + GetVolumeLabel(disk.GUID) + L"\r\n", resp, true);
