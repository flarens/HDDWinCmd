...
bool GetVolumeLettersContainingSwapFile(std::wstring& letters, s_resp& resp) {
	...
	if (RegQueryValueExW(hKey, L"ExistingPageFiles", nullptr, nullptr, nullptr, &size) != ERROR_SUCCESS) {
		...
		RegCloseKey(hKey);
		return true;	// если файл подкачки отключен, ключа ExistingPageFiles может не быть
	}
...
bool IsNotSystemDisk(const HANDLE hDevice, s_resp& resp) {
	...
	if (result < 0) {
		...
		WcoutExt(L"The selected disk contains the volume [" + std::wstring(1, systemVolumeLetter) + L"] with the current operating system, command canceled for security reasons\r\n", resp, false);
		return false;
	}
	...
	if (len > 0) {
		for (size_t i = 0; i < len; ++i) {
			...
			if (result < 0) {
				...
				WcoutExt(L"The selected disk contains the volume [" + std::wstring(1, volumeLetters[i]) + L"] where the swap file is located, command canceled for security reasons\r\n", resp, false);
				return false;
			}
		}
	}
...
// удалить/восстановить дисковое устройство
bool DiskUninstall(std::wstring devicePath, const int action, const DWORD timeout, s_resp& resp) {

	// только перманентные ссылки на диск
	if (!IsDevicePath(devicePath)) {
		WcoutExt(L"To execute commands with the disk device, use the permanent disk link\r\n", resp, false);
		return false;
	}
	...
	WcoutExt_Mini(L"Connection to HDDWinCmd Service...\r\n", resp, true);

	const wchar_t* pipeName = L"\\\\.\\pipe\\HDDWinCmd";
	HANDLE hPipe;

	// пробуем соединиться с пайпом
	hPipe = CreateFileW(pipeName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);

	// еще не существует
	if (hPipe == INVALID_HANDLE_VALUE) {

		// получаем путь к hddwincmd_service.exe
		std::wstring serviceAppPath;
		if (!GetServicePath(serviceAppPath, resp)) {
			return false;
		}

		// запускаем сервис
		if (!RunApplication(serviceAppPath, L"fromHDDWinCmd")) {
			WcoutExt(L"Failed to start the service [hddwincmd_service.exe]\r\n", resp, false);
			return false;
		}

		// ожидание подключения к пайпу
		int iter = 0;
		int iter_max = round(timeout / 1000) * 2;
		while (true) {
			...
			// другая ошибка кроме ненахождения пайпа	
			if (GetLastError() != ERROR_FILE_NOT_FOUND) {
				WcoutExt(L"Failed to connect to pipe. " + GetLastErrorString() + L"\r\n", resp, false);
				return false;
			}

			// выход по таймауту
			if (iter > iter_max) {
				WcoutExt(L"Failed to connect to pipe - connection timeout expired (timeout can be set with the command --timeout)\r\n", resp, false);
				return false;
			}
		}
	}

	WcoutExt_Mini(L"Command execution...\r\n", resp, true);

	// формируем комманду
	std::wstring message;
	if (action == 0) message = L"u";
	else if (action == 1) message = L"r";
	else if (action == 2) message = L"d";
	message += ConvertDevicePathToDeviceInstanceId(devicePath);

	// отсылаем комманду
	DWORD bytesWritten;
	if (!WriteFile(hPipe, message.c_str(), message.size() * sizeof(wchar_t), &bytesWritten, NULL)) {
		WcoutExt(L"Failed to write to pipe. " + GetLastErrorString() + L"\r\n", resp, false);
		CloseHandle(hPipe);
		return false;
	}
	...
	if (!ReadFile(hPipe, &response, sizeof(response), &bytesRead, NULL)) {
		WcoutExt(L"Failed to read from pipe. " + GetLastErrorString() + L"\r\n", resp, false);
		CloseHandle(hPipe);
		return false;
	}

	// распознаем результат
	switch (response) {
	case HDDSERV_SUCCESS:
		WcoutExt(L"Successfully completed\r\n", resp, true);
		break;
	case HDDSERV_UNKNOWN_COMMAND:
		WcoutExt(L"Service did not recognize the command\r\n", resp, false);
		break;
	case HDDSERV_ALREADY_DELETED:
		WcoutExt(L"The specified disk is already in the uninstalled list\r\n", resp, false);
		break;
	case HDDSERV_NOT_LOCATE_DEVNODE:
		WcoutExt(L"The specified disk could not be found in the device tree\r\n", resp, false);
		break;
	case HDDSERV_NOT_UNINSTALL_DEVNODE:
		WcoutExt(L"Failed to uninstall the specified disk\r\n", resp, false);
		break;
	case HDDSERV_NOT_REMOVE_SUBTREE:
		WcoutExt(L"Failed to delete all links of the specified disk\r\n", resp, false);
		break;
	case HDDSERV_NOT_REENUMERATE_DEVNODE:
		WcoutExt(L"Failed to reenumerate the specified disk\r\n", resp, false);
		break;
	case HDDSERV_LINK_NOT_FOUND:
		WcoutExt(L"No saved link to restore the selected disk\r\n", resp, false);
		break;
	default:
		WcoutExt(L"Unknown service response\r\n", resp, false);
		break;
	}
