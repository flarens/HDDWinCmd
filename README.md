...
## Утилита может предоставить следующую информацию о дисках:

Путь к диску вида

    \\.\PhysicalDrive0
    /dev/sda
    \\?\scsi#disk&ven_nvme&prod_samsung_ssd_980#5&d0a914e&0&000000#{53f56307-b6bf-11d0-94f2-00a0c91efb8b}

*(рекомендуется в командах использовать именно permanent device path, т.к. он постоянный)*

**Информация о диске:** тип шины, имя продукта и поставщика, версия, серийный номер, возможность извлечения, поддержка NCQ, геометрия диска, размер, тип таблицы разделов

**Информация о каждом логическом томе диска:** GPT GUID, volume GUID, буква диска, метка тома, файловая система, start LBA, размер, тип и имя партиции, GPT атрибуты
