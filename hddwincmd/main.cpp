...
int main(int argc, char* argv[]) {
	...
	for (int k = 1; k < argc; ++k) {
		...
		if (arg_k.rfind("--", 0) == 0) {
			...
			else if (arg_k == "--u" || arg_k == "--uninstall") {
				if (disk == L"") {
					WcoutExt(L"Error in [" + StringToWstring(arg_k) + L"] command: before executing the command, you must select the disk (using --disk)\r\n", resp, false);
				}else {
					if (params == 0) WcoutExt(L"Error in [" + StringToWstring(arg_k) + L"] command: parameter required\r\n", resp, false);
						...
						arg_p = argv[k + 1];
						if (arg_p == "0" || arg_p == "remove") DiskUninstall(GetPhysicalDriveFromDriveStr(disk), 0, timeout, resp);
						else if (arg_p == "1" || arg_p == "recovery") DiskUninstall(GetPhysicalDriveFromDriveStr(disk), 1, timeout, resp);
						else if (arg_p == "2" || arg_p == "delete") DiskUninstall(GetPhysicalDriveFromDriveStr(disk), 2, timeout, resp);
						else WcoutExt(L"Error in [" + StringToWstring(arg_k) + L"] command: unknown parameter [" + StringToWstring(arg_p) + L"]\r\n", resp, false);
					}
				}
			}
			...
			// запись лога
			else if (arg_k == "--l" || arg_k == "--log") {
				std::wstring logfilename = L"";
				if (params > 0) logfilename = StringToWstring(argv[k + 1]);
				resp.logger.SetLogFile(logfilename);
				WcoutExt(resp.logger.GetStatus() + L"\r\n", resp, true);
			}
